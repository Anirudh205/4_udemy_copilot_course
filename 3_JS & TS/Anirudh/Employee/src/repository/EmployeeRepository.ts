...
export class EmployeeRepository {
    ...
    public async getEmployees(): Promise<Employee[]> {
        const result = await this.connection.promise().query("SELECT * FROM employees");
        return result[0] as Employee[];
    }

    public async getEmployeeById(id: number): Promise<Employee | undefined> {
        const result = await this.connection.promise().query("SELECT * FROM employees WHERE id = ?", [id]);
